...
const StudentMessages = () => {
  ...
  const tutorId = localStorage.getItem("clickedStudentId");
  const studentId = localStorage.getItem("tutorId");
  ...
  return (
    ...
    >
      <Grid item xs={12} md={10} sx={{ mt: 10, width: '95%', display: 'flex', alignItems: 'center' }}>
        <IconButton onClick={() => navigate('/tutor/tutor-messages')} sx={{ mr: 2 }}>
        ...
        >
          <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 2 }}>
            ...
            {messages.map((msg) => {
              ...
              return (
                 <Box key={msg.messageId} display="flex" flexDirection="column" alignItems={isTutor ? "flex-end" : "flex-start"} mb={1}>
                    {!isTutor && (
                      <Typography variant="subtitle2" sx={{ alignSelf: 'flex-start', fontWeight: 'bold', mb: 0.5 }}>
                        {msg.student ? msg.student.firstName : 'Unknown Student'}
                      </Typography>
                    )}
                  <Box
                    sx={{
                      p: 1.5,
                      backgroundColor: isTutor ? '#213448' : '#94B4C1',
                      color: isTutor ? '#fff' : '#000',
                      borderRadius: 2,
                      maxWidth: '70%',
                      wordWrap: 'break-word',
                    }}
